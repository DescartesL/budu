...
public class ApiVersionCondition implements RequestCondition<ApiVersionCondition> {
    ...
    @Override
    public ApiVersionCondition combine(ApiVersionCondition another){
        return new ApiVersionCondition(another.getApiVersion());
    }
    ...
    private int compareVersion(String version1, String version2){
        ...
        int diff = 0;
        while(idx < minLen && (diff = versionArray1[idx].length() - versionArray2[idx].length()) == 0 && (diff = versionArray1[idx].compareTo(versionArray2[idx])) == 0) {
            idx ++;
        }
